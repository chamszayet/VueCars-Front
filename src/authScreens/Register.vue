    ...
    <form @submit.prevent="handleSubmit" class="mt-8 space-y-6" action="#" method="POST">
      ...
      <div class="rounded-md shadow-sm -space-y-px">
        ...
        <div>
          ...
          <input v-model="password2"  id="password2" name="password" type="password" autocomplete="current-password" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="re-Password">
        </div>
      </div>
    ...
    methods:{
      handleSubmit(){
        ...
        if(this.password !== this.password2){
          this.errors.push('password not similars')
        }
